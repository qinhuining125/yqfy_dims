...
public class BarcodeUtils {
    ...
    private static BufferedImage insertWords(BufferedImage image, String words) {
        ...
        if (words.length() != 0) {
            ...
            outg.setColor(color);
            // 字体、字型、字号
            outg.setFont(new Font("宋体", Font.PLAIN, 10));
            //文字长度
     ...
     */
    public static Boolean drawLogoQRCodeToFile(File codeFile, String qrUrl, String words) {
        Boolean flag=false;
        try {
            MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
     ...
     * @param qrUrl    二维码内容
     * @param words    二维码下面的文字
     * @description 生成带文字的二维码，直接返回到页面。
     */
    public static byte[] drawLogoQRCode(String qrUrl, String words) {
        try {
