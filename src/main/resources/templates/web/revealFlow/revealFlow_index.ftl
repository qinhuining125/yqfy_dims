  ...
  <div class="col-sm-12">
      ...
      <div class="ibox-content">
        <form class="form-horizontal" id="frm-search-taskFlow">
          <div class="row">
             <div class="col-xs-6 col-sm-4 col-lg-3">
              <div class="form-group">
                <label class="col-sm-4 control-label">revealId：</label>
                <div class="col-sm-7">
                  <input type="text" name="revealId" placeholder="搜索revealId" class="form-control" />
                </div>
              </div>
            </div>
             <div class="col-xs-6 col-sm-4 col-lg-3">
          ...
          <div class="text-center">
            <button id="btn-search-revealFlow" class="btn btn-primary"><i class="fa fa-search"></i> 查询</button>
    ...
    <div class="table-responsive ibox-content">
      <div class="clearfix" style="margin-bottom:10px;">
        <div class="pull-left">
          <button type="button" class="btn btn-primary" id="btn-add-revealFlow"><i class="fa fa-plus"></i> 新增</button>
          <button type="button" class="btn btn-danger" id="btn-del-revealFlow"><i class="fa fa-remove"></i> 删除</button>
        </div>
        <div class="grid-o pull-right" id="bootgrid-toolbar"></div>
      </div>
      <table id="table-revealFlow" class="table table-condensed table-hover table-striped table-bordered no-margins">
        <thead>
        <tr>
          <th data-column-id="ID" data-width="60px" data-formatter="no" data-sortable="false" data-identifier="true">序号</th>
           <th data-column-id="revealId" data-order="desc" data-visible="true" data-sortable="true">revealId</th>
...
<script id="template-revealFlow" type="text/x-handlebars-template">
  <td data-id="{{id}}">
...
<script id="template-view-taskFlow" type="text/x-handlebars-template">
  <div class="form-horizontal" style="margin: 20px;">
     <div class="form-group">
      <label class="col-xs-4 control-label">revealId：</label>
      <div class="col-xs-8">
        <div class="form-control-static">{{revealId}}</div>
      </div>
    </div>
     <div class="form-group">
...
<script>
  $(document).ready(function () {
    ...
    <#-- view的本页渲染的模板 -->
    var HBR_VIEW = Handlebars.compile($("#template-view-revealFlow").html());
    <#-- 列表ajax加载，绑定删除和编辑之外的其他按钮 -->
    function bindRowEvent() {
      <#-- 绑定查看，本例是用的本页渲染，也可以使用单独的页面 -->
      <#-- CommonGetData定义在main-list.js -->
      $("#table-revealFlow .ops-view").on("click", function () {
